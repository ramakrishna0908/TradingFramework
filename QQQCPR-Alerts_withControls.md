...
cprNarrowMult = input.float(1.0, "Narrow if today width < mult × N-day avg",
...
// Order: volume, RSI, CPR-width, VWAP, ATR-buffer
...
eUseCprWidth = preset == "Custom" ? useCprWidth :
preset == "Strict" ? true :
preset == "Conservative" ? true :
...
// === Signal generation (uses effective gate booleans) ===
...
rsiBull = not eUseRSI or rsi > 50
rsiBear = not eUseRSI or rsi < 50
volPass = not eUseVolume or volOK

    vwapBull = not eUseVwap or (not na(chartVwap) and c > chartVwap)
    vwapBear = not eUseVwap or (not na(chartVwap) and c < chartVwap)

    narrow = not eUseCprWidth or (not na(cprWavg) and cprW < cprWavg * cprNarrowMult)
...
if barstate.isconfirmed
table.cell(t, 0, 0, "Sym [" + preset + "]", text_color=color.yellow)
